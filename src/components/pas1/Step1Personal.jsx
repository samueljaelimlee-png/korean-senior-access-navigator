  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-sm p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        ...
        <div>
          <Label className="text-sm">이름 (First name) <span className="text-red-500 text-xs">필수</span></Label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        <div>
          <Label className="text-sm">생년도 <span className="text-red-500 text-xs">필수</span></Label>
      ...
      <div className="mb-3">
        <Label className="text-sm">주소 (Mailing address) <span className="text-red-500 text-xs">필수</span></Label>
        <Input
        ...
        {muniResult && (
          <div className="mt-2 p-2 bg-green-50 border border-green-500 rounded-lg flex items-center gap-2 text-sm text-green-800">
            ...
            <button onClick={applyMuni} className="px-3 py-1 bg-green-800 text-white rounded text-xs font-semibold">적용</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-muted rounded-lg p-3 mb-3 border border-border">
        ...
        <p className="text-xs text-muted-foreground mb-2">NJParcels.com에서 주소로 검색하면 바로 확인됩니다.</p>
        <a
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        <div>
          <Label className="text-sm">Municipality Code <span className="text-xs text-muted-foreground">(4자리)</span></Label>
        ...
        <div>
          <Label className="text-sm">SSN 뒷 4자리</Label>
      ...
      <div className="border-t border-secondary pt-3 mt-3">
        <div className="flex items-center gap-2 mb-3">
          ...
          <Label className="text-sm font-medium cursor-pointer">배우자 / Civil Union Partner 포함</Label>
        </div>
        {formData.hasSpouse && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <Label className="text-sm">배우자 성명 (영문)</Label>
              <Input placeholder="Kim, Jae Won" value={formData.spName} onChange={e => updateField('spName', e.target.value)} />
            </div>
            <div>
              <Label className="text-sm">배우자 생년도</Label>
