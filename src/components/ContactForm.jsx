...
export default function ContactForm({ onClose }) {
  ...
  const canSubmit = name.trim() && contact.trim() && inquiryType && message.trim() && agreed && !submitting;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
      <div className="bg-background w-full sm:max-w-lg sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[85vh]">
        {/* Header */}
        ...
        {done ? (
          ...
          <form onSubmit={handleSubmit} className="overflow-y-auto px-5 py-4 space-y-4 flex-1">
            ...
            {/* Inquiry type */}
            <div>
              <label className="text-sm font-semibold text-foreground mb-1.5 block">문의 유형</label>
            ...
            <label className="flex items-start gap-2.5 cursor-pointer p-3 bg-muted rounded-lg">
              ...
              <span className="text-xs text-foreground leading-relaxed">
                저는 Social Security Number, 은행정보, 세금보고서, 신분증 사진 등 민감정보를 보내지 않아야 함을 이해했습니다.
              </span>
            </label>
