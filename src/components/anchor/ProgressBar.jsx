...
const STEPS = [
  { label: '자격확인' }, { label: '기본정보' }, { label: '신고상태' },
  { label: '소득' }, { label: '거주' }, { label: '주택정보' },
  { label: '서명' }, { label: '완료' },
];

export default function ProgressBar() {
  const { formData, setStep } = useAnchor();
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <React.Fragment key={i}>
              <div className="flex flex-col items-center min-w-0 flex-1 cursor-pointer" onClick={() => { if (i <= current + 1) setStep(i); }}>
                <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold transition-all
                  ...
                  ${isActive ? 'bg-amber-600 text-white shadow-md ring-2 ring-amber-600/30' : ''}
                ...
                <span className={`text-[9px] sm:text-[10px] mt-1 text-center truncate max-w-[44px] sm:max-w-[56px]
                  ...
                  ${!isDone && !isActive ? 'text-muted-foreground' : ''}`}>{s.label}</span>
              </div>
              {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 mb-4 transition-colors ${i < current ? 'bg-accent' : 'bg-border'}`} />}
            </React.Fragment>
          );
        })}
      </div>
